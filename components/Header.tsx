...
const Header = () => {
    ...
    return (
        <header className='z-[999] relative w-full'>
            ...
            rounded-none border border-white border-opacity-40 bg-white/80
            shadow-lg shadow-black/[0.03] backdrop-blur-[0.5rem] sm:top-6
            sm:h-[3.25rem] sm:w-[42rem] sm:rounded-full dark:bg-gray-950
            ...
            py-2 sm:top-[1.7rem] sm:h-[initial] sm:py-0">
                <ul className="flex w-[22rem] justify-center
                items-center flex-wrap gap-y-1 text-[0.9rem]
                font-medium text-gray-500 sm:w-[initial]
                sm:flex-nowrap sm:gap-5">
                    {links.map((link, index) => (
                        <motion.li className="h-3/4 flex items-center relative
                    justify-center"
                            key={index}
                        ...
                        >
                            <Link href={link.hash}
                                className={clsx("flex items-center justify-center w-full p-3 hover:text-gray-950 transition dark:text-gray-500 dark:hover:text-gray-300",
                                    {
